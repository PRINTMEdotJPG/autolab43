...
function setupUI(app) {
    ...
    function initEventHandlers() {
        if (elements.startBtn) {
            elements.startBtn.addEventListener('click', function() {
                if (app.ws && app.ws.isConnected()) {
                    ...
                    app.log('Пожалуйста, дождитесь подключения WebSocket', 'warning');
                }
            });
        }
            ...
            elements.recordBtn.addEventListener('click', async function() {
                if (app.recording.isRecording()) {
                    ...
                    if (elements.recordingStatus) {
                        elements.recordingStatus.innerHTML = '<i class="bi bi-record-circle"></i> Запись: Не активна';
                    }
                } else {
                    try {
                        ...
                        if (elements.recordingStatus) {
                            elements.recordingStatus.innerHTML = '<i class="bi bi-record-circle"></i> Запись: Активна';
                        }
                    } catch (error) {
        ...
        if (elements.confirmParamsBtn && elements.paramsForm) {
            elements.paramsForm.addEventListener('submit', function(e) {
                e.preventDefault();
                ...
                if (app.validation.validateTemperature(temperature) && 
                    ...
                    if (app.ws.isConnected()) {
                        ...
                        app.log(`Параметры шага ${app.currentStep} сохранены`, 'success');
                    }
                }
            });
        }
